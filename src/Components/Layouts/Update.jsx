...
const Update = () => {
    ...
    const { user, updateMovieData } = useContext(AuthContext)
    ...
    useEffect(() => {
        setValue('title', updateMovieData.title)
        setValue('description', updateMovieData.description)
        setValue('Poster', updateMovieData.Poster)
        setValue('year', updateMovieData.year)
        setValue('genre', updateMovieData.genre)
        setValue('duration', updateMovieData.duration)
        setValue('rating', updateMovieData.rating)
    }, [updateMovieData?._id]);
    ...
    return (
        <div>
            <div className='text-center w-[94%] md:w-2/3 lg:w-1/2 mx-auto'>
                <h1 className='text-4xl font-semibold mt-16'>       Edit Your Movie Information</h1>
                <p className=' my-5'>Update the details of your favorite movies with ease! Modify information like the title, genre, release year, or description to ensure your movie collection stays accurate and up-to-date.</p>
            </div>
